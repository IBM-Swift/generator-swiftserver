...
exports.generatePackageSwift = function(config) {
  ...
  var store = '.Package(url: "https://github.com/IBM-Swift/GeneratedSwiftServer-CloudantStore.git", majorVersion: 0, minor: 4),';
  //TODO: re-implement this when we have added optional dependencies on the stores
  /*if (config.store === 'cloudant') {
    store = '.Package(url: "https://github.com/IBM-Swift/GeneratedSwiftServer-CloudantStore.git", majorVersion: 0, minor: 4),';
  }*/
  return `
...
exports.generateLocalConfig = function(specConfig, services) {
  ...
  Object.keys(services).forEach(function(serviceType) {
    ...
    services[serviceType].forEach(function(serviceDef) {
      config.services[serviceType].push(getLocalServiceConfig(serviceType, serviceDef));
    });
...
exports.generateCloudConfig = function(specConfig, services) {
  ...
  Object.keys(services).forEach(function(serviceType) {
    var type = convertToCloudType(serviceType);
...
function getLocalServiceConfig(serviceType, service) {
  switch(serviceType) {
    case ('cloudant'): return { name: service.name,
                                store: {
                                  type: serviceType,
                                  host: service.host || 'localhost',
                                  port: service.port || 5984,
                                  secured: service.secured || false,
                                  username: service.username,
                                  password: service.password
                                }
                              };
    case ('redis'): return { name: service.name,
                                store: {
                                  type: serviceType,
                                  host: service.host || 'localhost',
                                  port: service.port || 6397,
                                  username: service.username,
                                  password: service.password
                                }
                              };
...
function getCloudServiceConfig(serviceType, service) {
  switch(serviceType) {
    case ('cloudant'): return {
                                name: service.name,
                                label: convertToCloudType(serviceType),
                                tags: [],
...
function convertToCloudType(serviceType) {
  switch(serviceType) {
