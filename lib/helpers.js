  ...
  switch (serviceType) {
    case 'cloudant':
      ...
      if (service.credentials.url) {
        ...
        if (parsedURL.host) defaults.host = parsedURL.host
        if (parsedURL.port) defaults.host = parsedURL.port
        if (parsedURL.auth) {
      ...
      return {
        ...
        password: service.credentials.password || defaults.username,
        secured: service.credentials.secured || defaults.secured,
        port: service.credentials.port || defaults.port
      }
    case 'redis':
      var defaultURI = { protocol: 'redis', auth: ':', hostname: 'localhost', port: 6397, slashes: true }
      if (service.credentials.host) defaultURI.hostname = service.credentials.host
      if (service.credentials.port) defaultURI.port = service.credentials.port
      if (service.credentials.password) defaultURI.auth = `:${service.credentials.password}`
      return {
        uri: service.credentials.uri || url.format(defaultURI)
      }
    case 'objectstorage':
    ...
    case 'appid':
      ...
      return {
        ...
        version: service.credentials.version || defaultVersion
      }
    case 'watsonconversation':
      var defaultURL = 'https://gateway.watsonplatform.net/conversation/api'
      return {
        url: service.credentials.url || defaultURL,
        username: service.credentials.username || '',
        password: service.credentials.password || ''
      }
