...
exports.validateFilePath = function(path) {
  if (!path) {
    return 'Path is required';
  }
  if (!fs.existsSync(path)) {
    return format('The following path does not exist: %s', path);
  }
  return true
};
...
exports.generateCloudConfig = function(specConfig, services) {
  ...
  Object.keys(services).forEach(function(serviceType) {
    services[serviceType].forEach(function(service) {
      ...
      vcapServices[serviceLabel].push(getCloudServiceConfig(serviceType, service));
    });
  });
  return {
    vcap: {
...
exports.getBluemixServiceLabel = function(serviceType) {
  switch(serviceType) {
    ...
    case 'appid':               return 'AdvancedMobileAccess';
    case 'watsonconversation':  return 'WatsonConversation';
...
exports.ignoreFile = function(ignoreLine) {
  fs.appendFileSync('.gitignore', ignoreLine);
}
